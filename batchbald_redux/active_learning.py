...
__all__ = ['ActiveLearningData', 'get_balanced_sample_indices', 'get_subset_base_indices', 'get_base_indices',
           'get_base_dataset_index', 'get_base_dataset_indices', 'RandomFixedLengthSampler']
...
def get_base_dataset_index(dataset: data.Dataset, index: int):
    if isinstance(dataset, data.Subset):
        return get_base_dataset_index(dataset.dataset, dataset.indices[index])
    if isinstance(dataset, TransformedDataset):
        return get_base_dataset_index(dataset.dataset, index)
    if isinstance(dataset, data.ConcatDataset):
        dataset_index = bisect.bisect_right(dataset.cumulative_sizes, index)
        if dataset_index == 0:
            sample_index = index
        else:
            sample_index = index - dataset.cumulative_sizes[dataset_index - 1]
        return get_base_dataset_index(dataset.datasets[dataset_index], int(sample_index))
    return dataset, index


def get_base_dataset_indices(dataset: data.Dataset, indices: List[int], dataset_mapping: dict):
    base_dataset_indices = [get_base_dataset_index(dataset, index) for index in indices]

    dataset_indices = [(dataset_mapping[base_dataset], index) for base_dataset, index in base_dataset_indices]

...
__all__ = ['NamedDataset', 'OverridenTargetDataset', 'CorruptedLabelsDataset', 'RandomLabelsDataset',
           'get_class_indices', 'ImbalancedDataset', 'ImbalancedClassSplitDataset', 'OneHotDataset', 'RepeatedDataset',
           'SubsetDataset', 'ConstantTargetDataset', 'UniformTargetDataset', 'AdditiveGaussianNoise',
           'dataset_to_tensors', 'get_dataset_state_dict', 'ImportedDataset', 'save_dataset', 'load_dataset',
           'get_base_dataset', 'get_target', 'get_targets', 'create_repeated_MNIST_dataset', 'create_MNIST_dataset']
    ...
    def __init__(self, dataset: data.Dataset, *, size: Optional[int] = None, factor: Optional[float] = None, seed: int):
        ...
        if size is not None:
            ...
            if seed == 0:
                ...
                self.alias = f"{dataset}[:{size};seed={seed}]"
        elif factor is not None:
            subset_size = int(len(dataset) * factor)
            if seed == 0:
                self.alias = f"{dataset}~x{factor}"
            else:
                self.alias = f"{dataset}~x{factor} (seed={seed})"

...
def evaluate_online_bayesian_inference(model: BayesianModule, *, real_training_set_size: int,
    ...
    return eval_obi_simple(log_probs_N_M_C=log_probs_N_M_C, labels_N=labels_N,
                           num_training_indices=num_training_indices,
...
def eval_obi_simple(log_probs_N_M_C, labels_N, num_training_indices, start_index, num_samples_list, num_trials,
    ...
    for online_training_set_size in range(start_index, num_training_indices + 1):
        for num_samples in num_samples_list:
            for trial in range(num_trials):
                ...
                # mean instead of sum.
                mix_weights_m = torch.mean(true_train_log_probs_n_m, dim=0)
                mix_weights_m_max = torch.max(mix_weights_m)

                # simply take the top 10.
                top_10_samples = torch.topk(mix_weights_m, k=10)[1]
                ...
                joint_log_probs_n_m_C = (mix_weights_m[None, top_10_samples, None] - mix_weights_m_max
                                         + trial_log_probs_N_m_C[num_training_indices:, top_10_samples])
                del mix_weights_m
                ...
                accuracy = torch.mean(
                    (predictions_n == labels_N[num_training_indices:]).type(torch.float)).cpu().item()
